...
class TrafficSignal:
    ...
    def get_waiting_time_of_lane(self):
        vehicle_size_min_gap = 7.5  # 5(vehSize) + 2.5(minGap)
        return [traci.lane.getWaitingTime(lane) / (traci.lane.getLength(lane) / vehicle_size_min_gap) for lane in self.lanes]
    ...
    def _get_veh_list_per_lane(self, p):
        veh_list = []
        for lane in self.lanes[p]:
            veh_list += traci.lane.getLastStepVehicleIDs(lane)
        return veh_list
