...
class SumoEnvironment(MultiAgentEnv):
    ...
    def __init__(self, net_file, route_file, phases, out_csv_name=None, use_gui=False, num_seconds=20000, max_depart_delay=100000,
                 time_to_load_vehicles=0, delta_time=5, yellow_time=2, min_green=5, max_green=50, single_agent=False, ggi=False, 
        ...
        self.reward_space = spaces.Discrete(4)
    ...
    def _compute_observations(self):
        ...
        for ts in self.ts_ids:
            ...
            waiting_ = self.traffic_signals[ts].get_waiting_time_of_lane()
            observations[ts] = phase_id + waiting_ + density             # + [elapsed]
        return observations

    def _compute_rewards(self):
        if self.ggi:
            return self._waiting_time_reward_GGI()
    ...
    def _waiting_time_reward_GGI(self):
        rewards = {}
        for ts in self.ts_ids:
            reward = np.array(self.traffic_signals[ts].get_waiting_time())
            rewards[ts] = -1 * reward
            self.last_measure[ts] = reward
        return rewards

    def _waiting_time_reward(self):
        rewards = {}
        for ts in self.ts_ids:
            ts_wait = np.array(self.traffic_signals[ts].get_waiting_time())
            rewards[ts] = -1 * np.sum(ts_wait)
    ...
    def _waiting_time_reward3(self):
        ...
        for ts in self.ts_ids:
            ts_wait = sum(self.traffic_signals[ts].get_waiting_time())
            rewards[ts] = -ts_wait
            rewards[ts] = [-ts_w for ts_w in ts_wait]
            self.last_measure[ts] = ts_wait
        return rewards
    ...
    def _obs_wait_time(self):
        veh_list = []
        for ts in self.ts_ids:
            for p in range(self.num_green_phases):
                veh_list_each = self.traffic_signals[ts]._get_veh_list(p)
                veh_list.append(veh_list_each)
            return veh_list

    def _compute_step_info(self):
        a2 = [self.last_measure[ts] for ts in self.ts_ids][0] 
        return {
            'step_time': self.sim_step,
            'total_stopped': sum([sum(self.traffic_signals[ts].get_stopped_vehicles_num()) for ts in self.ts_ids]),
            'total_wait_time': np.sum(a2),
            'total_wait_time1': int(a2[0]),
            'total_wait_time2': int(a2[1]),
            'total_wait_time3': int(a2[2]),
            'total_wait_time4': int(a2[3])
        }
