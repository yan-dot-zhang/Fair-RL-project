...
class PPO2(ActorCriticRLModel):
    ...
    def setup_model(self):
        with SetVerbosity(self.verbose):
            ...
            self.reward_n = self.reward_space
            ...
            with self.graph.as_default():
                ...
                with tf.variable_scope("loss", reuse=False):
                    ...
                    self.advs_ph = tf.placeholder(tf.float32, [None, self.reward_n], name="advs_ph")
                    self.rewards_ph = tf.placeholder(tf.float32, [None, self.reward_n], name="rewards_ph")
                    self.old_neglog_pac_ph = tf.placeholder(tf.float32, [None], name="old_neglog_pac_ph")
                    self.old_vpred_ph = tf.placeholder(tf.float32, [None, self.reward_n], name="old_vpred_ph")
                    self.learning_rate_ph = tf.placeholder(tf.float32, [], name="learning_rate_ph")
                    self.clip_range_ph = tf.placeholder(tf.float32, [], name="clip_range_ph")
                    self.ini_obs_value = tf.placeholder(tf.float32, [self.reward_n], name="initial_obs_value")
                    self.sor_omega = tf.placeholder(tf.float32, [self.reward_n], name="self.sor_omega")
    ...
    def _train_step(self, learning_rate, cliprange, avg_init_states, sorted_omega, obs, returns, masks, actions, values, neglogpacs, update,
        ...
        :params sorted_omega: sorted ggi vector w.r.t to init state
    ...
    def learn(self, total_timesteps, callback=None, log_interval=1, tb_log_name="PPO2",
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            for update in range(1, n_updates + 1):
                ...
                obs, avg_value, w, returns, masks, actions, values, neglogpacs, states, ep_infos, true_reward = runner.run()
...
class Runner(AbstractEnvRunner):
    def __init__(self, *, env, model, n_steps, gamma, lam, reward_n, ggi_constant):
        """
        ...
        self.ggi_constant = ggi_constant
    ...
    def run(self):
        ...
        last_gae_lam = np.zeros(self.reward_n)
        for step in reversed(range(self.n_steps)):
            if step == self.n_steps - 1:
                nextnonterminal = np.expand_dims((1.0 - self.dones), axis=1)
                nextvalues = last_values
            else:
                nextnonterminal = np.expand_dims((1.0 - mb_dones[step + 1]), axis=1)
        ...
        return mb_obs, averaged_state_value, w, mb_returns, mb_dones, mb_actions, mb_values, mb_neglogpacs, mb_states, ep_infos, true_reward
